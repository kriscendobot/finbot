  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        forecast: { type: 'object', description: "the forecaster projection carrying p05Equity (the tail-risk anchor for the fire-time audit) and horizon. When config.dataSufficiencyMinCoverage arms the data-sufficiency gate the fire-time re-audit reads the forecaster's dataSufficiency descriptor too, and fails CLOSED without it; pass it through from the projection that was cited rather than building one, since the gate bounds forgery, not provenance" },
