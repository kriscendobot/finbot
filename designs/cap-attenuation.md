...
## Notes from the field (2026-07-11, AUDIT stage by inference)

The inference-driven OODA axis reached its safety gate: the **AUDIT stage now runs by inference** alongside the already-wired ORIENT (analyzer) and DECIDE (planner) dispatches. `dispatchAuditor` (`packages/pipeline/role-dispatch.js`) spawns the auditor as an LLM-shaped subagent over the planner's proposal plus a Monte Carlo forecast; it reasons in natural language, then **CALLS the deterministic `audit_proposal` gate** (`audit` wrapped as a harness tool in `agent-tools.js`, capability set `AUDITOR_TOOL_NAMES`) rather than adjudicating the invariants by hand. The verdict — approved/rejected with the six per-invariant results — is extracted from the tool-execution events, so the inference path and the headless `runOodaCycle` gate yield the *same* verdict.

- **Cap-attenuation angle.** The point the design makes here is that the gate reasons by inference yet stays strictly read-only: the audit-phase tool subset is `['audit_proposal']` and nothing in it can name a wallet, a signer, or a keystore — a verdict is a precondition for a live executor dispatch, never an authorization. The auditor recomputes the planner's `proposal_hash` inside the gate, so an inference-reordered or hand-edited proposal cannot slip an unreproducible plan past the gate (the reproducibility invariant fails closed).
- **Determinism proof.** `packages/pipeline/test/auditor-dispatch.test.js` asserts the inference-driven verdict `deepEqual`s the headless `audit(...)` verdict byte-for-byte, and that a tail-floor-busting proposal is *rejected* through the same gate (not only the happy path).
- **Chained in the bin.** `bin/finbot-dispatch` now drives OBSERVE → ORIENT → DECIDE → AUDIT entirely by inference in one command (verified `seed=7` → 1-step proposal `hash=7c90a9a577e7…`, verdict **approved**, all 6 invariants pass). Still dry-run and wallet-free by construction.

What remains on the axis: **ACT (executor) by inference**, which is the one stage whose live mode touches the wallet — its dry-run form can be wired read-only like the others, but a live executor dispatch stays gated behind cap-attenuation Phase 2 (§ Process boundary; the `spawnSigningWorker` stub) and explicit per-job `live_authorized: true`.

## Notes from the field (2026-07-12, ACT stage by inference — dry-run)

The inference-driven OODA axis is now **complete end to end**: the **ACT stage's executor runs by inference in dry-run**, joining the already-wired ORIENT (analyzer), DECIDE (planner), and AUDIT (auditor) dispatches. `dispatchExecutor` (`packages/pipeline/role-dispatch.js`) spawns the executor as an LLM-shaped subagent over the audited proposal; it reasons in natural language, then **CALLS the deterministic `simulate_execution` tool** (`execute` pinned to dry-run, wrapped as a harness tool in `agent-tools.js`, capability set `EXECUTOR_TOOL_NAMES`) rather than adjusting balances by hand. The tool re-runs the audit invariants at fire time (the drift guard), simulates the approved steps against a CLONE of the portfolio (the live book is never mutated), and builds the would-be substrate transaction; the `ExecutionResult` is extracted from the tool-execution events, so the inference path and the headless `runOodaCycle` executor yield the *same* dry-run.

- **Cap-attenuation angle.** This is the one OODA stage whose *live* mode touches the wallet, so the dry-run wiring is deliberately fail-safe by construction — not by convention. The `simulate_execution` tool **hard-codes `mode: 'dry-run'` and vends no `parentCaps`**, so `execute` runs the capability attenuator with an empty parent set: the executor's dry-run branch asserts `caps.wallet === undefined` and carries `walletTouched: false` as the proof. There is no tool argument that can request `mode: 'live'`, a keystore, or a wallet — the inference-driven executor has no reachable path to a signer. A *live* executor dispatch (a real wallet Far behind CapTP) remains gated behind cap-attenuation Phase 2 (§ Process boundary; the `spawnSigningWorker` stub) and explicit per-job `live_authorized: true`.
- **Determinism proof.** `packages/pipeline/test/executor-dispatch.test.js` asserts the inference-driven dry-run reproduces the headless `execute(...)` byte-for-byte — the simulated steps, the post-execution balances, the prepared substrate transaction, the target substrate, and the fire-time audit verdict — and that a fire-time-audit rejection (tail floor above the forecast p05) surfaces with zero simulated steps and the wallet still untouched (not only the happy path).
