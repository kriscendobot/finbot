...
7. **Forecast data-sufficiency** (opt-in; OFF unless the operator sets a minimum coverage ratio). A projection whose horizon outruns its observed window is extrapolating past its evidence — the pre-execution sibling of pricing freshness, since a forecast can be fresh and still be thin. Armed, the gate fails CLOSED, and it bounds forgery rather than provenance, so the descriptor must come from the projection that was cited and never be hand-built; [pre-execution-audit](../../skills/pre-execution-audit/SKILL.md) § 7 is the canonical statement of both.
